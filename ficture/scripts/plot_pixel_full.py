...
def plot_pixel_full(_args):
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="If the input contains categorical labels instead of probabilities",
    )
    parser.add_argument(
        "--category_map",
        type=str,
        default="",
        help='If needed, map the input category to a different set of names matching that specified in the color table. The mathcing could be multiple to one, input should have two columns as "From" and "To"',
    )
    ...
    parser.add_argument(
        "--input_rgb_uint8",
        action="store_true",
        help="If input rgb is from 0-255 instead of 0-1",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="Actual size (um) corresponding to each pixel in the output image",
    )
    parser.add_argument(
        "--org_coord",
        action="store_true",
        help="If the input coordinates do not include the offset (if your coordinates are from an existing figure, the offset is already factored in)",
    )
    ...
    color_info = pd.read_csv(
        args.color_table,
        sep=sep,
        header=0,
        index_col=args.color_table_index_column,
        dtype=cdty,
    )
    ...
    for df in loader:
        if df.shape[0] == 0:
            continue
        df["X"] = np.clip(
            ((df.X - loader.xmin) / args.plot_um_per_pixel).astype(int), 0, width - 1
        )
        df["Y"] = np.clip(
            ((df.Y - loader.ymin) / args.plot_um_per_pixel).astype(int), 0, height - 1
        )
        if categorical:
            if args.debug:
                print(
                    df.loc[~df[args.category_column].isin(category_map), :][
            ...
            if args.plot_top:
                ...
                for c in rgb:
                    ...
                    for k in range(1, loader.meta["TOPK"] + 1):
                        df[c] += (
                            color_info.loc[df["K" + str(k)].values, c].values
                            * df["P" + str(k)].values
                        )
        df = df.groupby(by=["X", "Y"]).agg({c: "mean" for c in rgb}).reset_index()
        logging.info(
            f"Reading pixels... {df.X.iloc[-1]}, {df.Y.iloc[-1]}, {df.shape[0]}"
        )
...
if __name__ == "__main__":
    ...
    # Define base paths with ~
    data_dir = "/data/yosef2/martinak/XeniumIPF225721"
    n_factor = 12
    pixel_resolution = 0.5
    output_id = f"nF{n_factor}.d_12"
    output_path = f"{data_dir}/ficture_output/analysis/{output_id}"
    figure_path = f"{output_path}/figure2"
    prefix = f"{output_id}.decode.prj_12.r_4_5"
    cmap = f"{output_path}/figure/{output_id}.rgb.tsv"
    input_file = f"{output_path}/{prefix}.pixel.sorted.tsv.gz"
    output_file = f"{figure_path}/{prefix}.pixel.png"
